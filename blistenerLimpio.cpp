    ...
    try{
        ...
        if(datagrama[0] == "CQ"){
            ...
            if(bconect.IdBConnect == 0){
                ...
                //la mac existe pero esta en otra sala, se inserta en lounge pasa a estatus 11 bloqueado.
                if(respuesta.first == -1){
                     bconect = Transacciones::MvBConnects::Nuevo(respuesta.second, datagrama[1], 11);
                }
                //la mac no existe se inserta en central y en lounge, pasa a estatus 12 pendiente 
                else if(respuesta.first == 1){
                    bconect = Transacciones::MvBConnects::Nuevo(respuesta.second, datagrama[1], 12);
                }
                //la mac si existe en central y es la misma sala, se inserta en lounge pasa a estatus 10 Conectado.
 ...
 * @param idConect 
 */
void sendMessage(const string& mensaje, const string& tipo, const int& clientSocket, const int& idConect = 0) {
   Seguridad::Aes3 objEnc;
   ...
   if (tipo == "A") {
       ...
       cout << "Mensaje para todos espera respuesta: "+Bconects.size() << endl;
       if(Bconects.size() > 0 ){ 
            ...
            string resDatagrama = objEnc.Encriptar("Mensaje procesado correctamente");
            //const char* sdata = resDatagrama.c_str();
            send(clientSocket, resDatagrama.c_str(), resDatagrama.length(),0);
       }else{
            cout << "No hay Bconects en linea :("<<endl;
            string resDatagrama = objEnc.Encriptar("No hay Bconects en linea :(");
            //const char* sdata = resDatagrama.c_str();
            send(clientSocket, resDatagrama.c_str(), resDatagrama.length(),0);
       ...
       if (it != Bconects.end()) {
           ...
           cout << "Mensaje específico enviado al BConect: " << idConect << endl;
            string resDatagrama = objEnc.Encriptar("Mensaje específico enviado al BConect: "+to_string(idConect));
            //const char* sdata = resDatagrama.c_str();
            send(clientSocket, resDatagrama.c_str(), resDatagrama.length(),0);
       } else {
            cout << "No se encontro el big conect :( "<<endl;
            string resDatagrama = objEnc.Encriptar("No hay Bconects en linea :(");
            //const char* sdata = resDatagrama.c_str();
            send(clientSocket, resDatagrama.c_str(), resDatagrama.length(),0);
       }
